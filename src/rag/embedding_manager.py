...
Оптимизирует производительность за счёт кэширования модели и поддерживает
два режима обработки: одиночный и пакетный. Логирует ключевые операции
через встроенный логгер.
...
class EmbeddingService:
    ...
    def generate_embedding(
        ...
        """
        Генерирует эмбеддинг для текста или списка текстов.

        Поддерживает:
        * одиночную обработку (str → List[float]);
        * пакетную обработку (List[str] → List[List[float]]).

        Логирует количество обрабатываемых текстов и завершение операции.
        """
